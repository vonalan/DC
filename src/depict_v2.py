...
class NeuralNetwork(object):
    def __init__(self, indim=4096, outdim=6):
        ...
        self.O = tf.multiply(self.T, self.Q)
    ...
    def predict(self, X):
        cost, Y = self.sess.run([self.cost, self.P], feed_dict={self.X: X})
        return cost, Y
    ...
    def saveModel(self, dstdir, epoch):
        ...
        saver.save(self.sess, os.path.join(dstdir, 'cluster_%d.ckpt'%epoch))

    def loadModel(self, dstdir, epoch):
        import os
        saver = tf.train.Saver()
        saver.restore(self.sess, os.path.join(dstdir, 'cluster_%d.ckpt'%epoch))
