...
def build_text_line_reader(filenames=None, shuffle=False, batch_size=1):
    ...
    return dict(filenames=filenames, iterator=iterator, next_elements=next_elements)
...
def build_train_graph(defalut_inputs, input_dim, output_dim, func=''):
    ...
    return dict(inputs=inputs, cost=cost, optimizer=optimizer)

class DepictTrainModel(object):
    def __init__(self, iterator, input_dim=2048, output_dim=10):
        self.graph = build_train_graph(iterator, input_dim, output_dim)
...
def main():
    ...
    for i in itertools.count():
        ...
        _, training_cost, train_summary = train_sess.run([optimizer, train_cost, train_merger],
                                                         feed_dict={train_input: xs_train})
